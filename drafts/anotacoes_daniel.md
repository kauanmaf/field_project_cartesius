...
You may have noticed that the triple-barrier method is path-dependent. In order to label an observation, we must take into account the entire path spanning $[t_{i,0}, t_{i,0} + h]$, where $h$ defines the vertical barrier (the expiration limit). We will denote $t_{i,1}$ the time of the first barrier touch, and the return associated with the observed feature is r_{t_{i,0},t_{i,1}} . For the sake of clarity, $t_{i,1} \le t_{i,0} + h$ and the horizontal barriers are not necessarily symmetric.
...
triple barrier. These are the timestamps selected by the sampling procedures discussed in Chapter 2, Section 2.5.
